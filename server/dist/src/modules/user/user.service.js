...
let UserService = class UserService {
    ...
    async updateDistributor(id, data) {
        ...
        if (data.nickname || data.status !== undefined) {
            const userPayload = {};
            if (data.nickname)
                userPayload.nickname = data.nickname;
            const statusInt = toStatusInt(data.status);
            if (statusInt !== undefined)
                userPayload.status = statusInt;
    ...
    async auditDistributor(id, pass, remark) {
