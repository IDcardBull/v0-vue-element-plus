...
exports.calcShippingByTemplate = calcShippingByTemplate;
function asArray(v) {
    if (Array.isArray(v))
        return v;
    if (typeof v === 'string') {
...
function regionMatch(regions, province) {
    // regions 为空或未传 → 全国适用
...
function pickRule(template, province) {
    const specials = asArray(template.specialRules);
 ...
 */
function calcShippingByTemplate(template, items, province) {
    if (!items?.length)
        return 0;
    const units = Number(template.calcType) === 2
    ...
    if (template.freeShippingEnabled) {
        ...
        const rules = asArray(template.freeShippingRules);
        const hit = rules.find((r) => regionMatch(r.regions, province) && subtotal >= (Number(r.threshold) || Infinity));
        if (hit)
            return 0;
    }
    const rule = pickRule(template, province);
    if (rule.firstAmount <= 0)
        return 0; // 模板未配置首件，按免邮处理
    if (units <= rule.firstAmount)
        return rule.firstPrice;
    if (rule.continueAmount <= 0)
        return rule.firstPrice; // 没配续费，只收首费
...
//# sourceMappingURL=shipping-template.calc.js.map