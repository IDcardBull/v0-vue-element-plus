...
var WechatPayNotifyController_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.WechatPayNotifyController = void 0;
 ...
 */
let WechatPayNotifyController = WechatPayNotifyController_1 = class WechatPayNotifyController {
    constructor(prisma, orderSvc, wxpay) {
        this.prisma = prisma;
        this.orderSvc = orderSvc;
        this.wxpay = wxpay;
        this.logger = new common_1.Logger(WechatPayNotifyController_1.name);
    }
    async payCallback(req, res) {
        ...
        try {
            ...
            this.logger.log(`[WxPayNotify] out_trade_no=${result.out_trade_no} trade_state=${result.trade_state} tx=${result.transaction_id}`);
            ...
            if (!order) {
                ...
                this.logger.warn(`[WxPayNotify] 订单不存在或已删除: ${result.out_trade_no}`);
                return this.ack(res);
            }
            if (order.status === 'pending_pay') {
                try {
                    ...
                    this.logger.warn(`[WxPayNotify] markPaid 触发幂等冲突: ${err?.message || err}`);
                }
            }
            else {
                this.logger.log(`[WxPayNotify] 订单 ${order.orderNo} 当前状态=${order.status}，跳过 markPaid`);
            }
            return this.ack(res);
        }
        catch (err) {
            ...
            // （这种通常是配置问题，不要静默吞掉）
            this.logger.error(`[WxPayNotify] 处理失败: ${err?.message || err}`, err?.stack);
            return res
                .status(401)
...
exports.WechatPayNotifyController = WechatPayNotifyController;
__decorate([
    (0, public_decorator_1.Public)(),
    (0, common_1.Post)('pay-callback'),
...
], WechatPayNotifyController.prototype, "payCallback", null);
exports.WechatPayNotifyController = WechatPayNotifyController = WechatPayNotifyController_1 = __decorate([
    (0, common_1.Controller)('wechat'),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService,
        order_service_1.OrderService,
        wechat_pay_service_1.WechatPayService])
], WechatPayNotifyController);
//# sourceMappingURL=wechat-pay-notify.controller.js.map