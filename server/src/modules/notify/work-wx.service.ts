 ...
 */
@Injectable()
export class WorkWxService {
  private readonly logger = new Logger(WorkWxService.name)

  /** 群机器人允许的最大字节数（实际限制 4096，留出 buffer） */
  private static readonly MAX_BYTES = 3500
  ...
  private getWebhook(): string {
    return (this.config.get<string>('WORK_WX_BOT_WEBHOOK') || '').trim()
  }
  ...
  private async post(payload: any): Promise<void> {
    ...
    try {
      ...
      if (data?.errcode && data.errcode !== 0) {
        this.logger.warn(
          `[WorkWx] 推送失败 errcode=${data.errcode} errmsg=${data.errmsg}`,
        )
      }
    } catch (err: any) {
  ...
  private trim(content: string): string {
    ...
    if (buf.byteLength <= WorkWxService.MAX_BYTES) return content
    return buf.slice(0, WorkWxService.MAX_BYTES).toString('utf8') + '\n...(已截断)'
  }

  // ============ 高层方法 ============

  /** 纯文本通知（@everyone 用 @all） */
  async sendText(content: string, mentionAll = false) {
    await this.post({
      msgtype: 'text',
      text: {
  ...
  async sendCreditOrderCreated(order: {
    ...
    const usageColor =
      usagePct >= 90 ? 'warning' : usagePct >= 70 ? 'comment' : 'info'
  ...
  async sendLogisticsTerminal(params: {
    ...
    company: string
    trackingNo: string
    state: 'signed' | 'rejected' | 'returned' | 'problem'
    lastContext?: string
  }) {
    if (!this.isEnabled()) return
